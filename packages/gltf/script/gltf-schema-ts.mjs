...
// Generate index file
const indexFile = Object.keys(schemas).reduce((indexFile, fileName) => {
  return `${indexFile}export * as ${fileName} from './${fileName}';\n`;
}, '');
fs.writeFile(`${outputPath}/index.ts`, indexFile, (err) => {
  if (err) throw err;
