import { GenIdMap, SparseSetMap } from '../map';
...
describe.each([
  ...
  test('clear() should empty the container', () => {
    ...
    map.set(GenId.create(0, 1), '1');
    map.set(GenId.create(1, 2), '2');
  ...
  test('delete() should do nothing and return false for non-existent id', () => {
    const map = new MapType<string>();
    map.set(GenId.create(0, 1), 'hello');
  ...
  test('foreach() should loop through all id-values', () => {
    ...
    const id1 = GenId.create(0, 1), id2 = GenId.create(10, 2);
    map.set(id1, value1);
    map.set(id2, value2);
